...
authRouter.post("/register", registerLimiter, async (req, res) => {
  ...
  const info = db
    .prepare(
      "INSERT INTO users (email, password_hash, name, verify_token_hash, verify_token_expires) VALUES (?, ?, ?, ?, ?)"
    )
    .run(normalizedEmail, password_hash, name.trim(), hashToken(rawVerifyToken), verifyExpires);

  deliverLink("Email verification", normalizedEmail, `${FRONTEND_URL}/#/verify-email/${rawVerifyToken}`);

  const user = { id: info.lastInsertRowid, email: normalizedEmail, name: name.trim(), token_version: 0 };
...
authRouter.get("/me", requireAuth, (req, res) => {
  const row = db.prepare("SELECT id, email, name, email_verified, created_at FROM users WHERE id = ?").get(req.user.sub);
...
authRouter.post("/forgot-password", tokenLimiter, (req, res) => {
  ...
  const normalizedEmail = email.trim().toLowerCase();
  const row = db.prepare("SELECT id, email FROM users WHERE email = ?").get(normalizedEmail);
  if (row) {
    const rawToken = generateRawToken();
    const expires = new Date(Date.now() + RESET_TOKEN_TTL_MS).toISOString();
    db.prepare("UPDATE users SET reset_token_hash = ?, reset_token_expires = ? WHERE id = ?").run(hashToken(rawToken), expires, row.id);
...
authRouter.post("/reset-password", tokenLimiter, async (req, res) => {
  ...
  const row = db.prepare("SELECT id, reset_token_expires FROM users WHERE reset_token_hash = ?").get(tokenHash);
...
authRouter.post("/change-password", requireAuth, async (req, res) => {
  ...
  db.prepare("UPDATE users SET password_hash = ?, token_version = token_version + 1 WHERE id = ?").run(password_hash, row.id);
  res.json({ message: "Password changed. Please log in again." });
});

// Issues a fresh verification link (the original expires after 24h).
authRouter.post("/resend-verification", requireAuth, tokenLimiter, (req, res) => {
  ...
  db.prepare("UPDATE users SET verify_token_hash = ?, verify_token_expires = ? WHERE id = ?").run(hashToken(rawToken), expires, row.id);
...
authRouter.post("/verify-email", tokenLimiter, (req, res) => {
  const { token } = req.body || {};
  if (!token) return res.status(400).json({ error: "Missing verification token." });

  const tokenHash = hashToken(token);
  const row = db.prepare("SELECT id, verify_token_expires FROM users WHERE verify_token_hash = ?").get(tokenHash);
  if (!row || isTokenExpired(row.verify_token_expires)) {
    return res.status(400).json({ error: "This verification link is invalid or has expired." });
  }

  db.prepare("UPDATE users SET email_verified = 1, verify_token_hash = NULL, verify_token_expires = NULL WHERE id = ?").run(row.id);
